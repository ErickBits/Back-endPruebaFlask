...
@app.route('/añadir_datos/usuario', methods=['POST'])
def añadir_datos():
...
#especificamos la ruta iniciarSesion por medio de la url 
...
def session():
        if request.method == 'POST':
                usuario = request.form['usuario']
                contaseña = request.form['Pass']

                cur = mysql.connection.cursor()
                resultado = cur.execute('SELECT * FROM usuario usuario = %s ', [usuario])
                
                if resultado > 0:
                        user = cur.fetchone()
                        Pass = user['Pass']

                        if sha256_crypt.verify('contraseña, Pass'):
                                session['logged_in'] = True
                                session['usuario'] = usuario
                                flash('Estas dentro ', 'succes')
                                app.logger.info('contraseña resivida')
                                return redirect('/admin')

                else:

                                app.logger.info('Contraseña incorrecta')
                                error = 'lo sentimos, ocurrio un problema'
                        ...
                        app.logger.info('usuario no encontradp')
...
#especificamos la ruta inicio por medio de la url 
@app.route('/Inicio')
def Inicio():
